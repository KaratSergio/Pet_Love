...
const MobNav: React.FC<{ className?: string }> = ({ className }) => {
  ...
  return (
    <>
      ...
      <button
        onClick={toggleMenu}
        className="absolute top-6 right-5 sm:right-[32px] sm:top-[24px] text-white rounded-full lg:hidden"
        aria-label="Open menu"
      >
        <Icon id="icon-menu" strokeColor={menuIconStroke} className="w-8 h-8 sm:w-9 sm:h-9" />
      </button>
      ...
      >
        <button onClick={toggleMenu} className="absolute top-4 right-4 p-2" aria-label="Close menu">
          <Icon id="icon-close" strokeColor={closeIconStroke} className="w-8 h-8 sm:w-9 sm:h-9" />
        </button>
